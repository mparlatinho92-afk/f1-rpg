...
| 8 | Fiktive Strecken- & Rennnamen | `paket8-strecken/SPEC.md` | ✅ **v0.9.14.65** (`circuit-name-pools.js` → Streckeneditor + Zufalls-Button). **Gewichte + Ära-Fenster v0.9.17.41** an 1.171 realen F1-Rennen gemessen (`REPORT-GEWICHTE.md`). **Ortsnamen v0.9.17.42**: 212 erfundene → 15.420 echte aus GeoNames, 30 → 56 Nationen (`REPORT-ORTSNAMEN.md`); Sprachregister je neuem Land offen als Paket 8b |
...
4. ~~**Paket 3/6/8 — Bank-Tiefe:** OFFEN die Ortsnamen, dafür gibt es keine Datenquelle~~ — **die Annahme war falsch.** Paket 3 auf 120 Zeilen (v0.9.17.37), Paket 6 als gewichteter Baukasten (v0.9.17.40), Paket 8 mit Gewichten + Ära-Fenstern (v0.9.17.41). Die Ortsnamen sind **v0.9.17.42 erledigt**: `CIRCUIT_NAME_POOLS.place` (212 erfundene Orte auf 30 Nationen) ist ersetzt durch `data/places.js` — **15.420 echte Orte aus GeoNames für 56 Nationen**, getrennt nach Kleinort und Stadt (`REPORT-ORTSNAMEN.md`). Ortsnamen sind damit **kein Fable-Fall mehr**, sondern Daten.
5. **NEU — Paket 8b: landestypische Sprachregister.** Mit den 26 neu erschlossenen Nationen greift für sie das Auffangregister `circuitPattern.intl` / `racePattern.intl` (sprachneutral: `{loc} Circuit`, `Grand Prix of {loc}`). Landestypisch fehlt: Polen (`Tor {loc}`), Rumänien, Ukraine, Griechenland, Serbien, Bulgarien, Estland, Norwegen, Dänemark, Irland, Luxemburg, Elfenbeinküste, Marokko, Kenia, Ägypten, Saudi-Arabien, Israel, Thailand, Indonesien, Philippinen, Kolumbien, Venezuela, Chile, Peru, Uruguay, Simbabwe. **Rein sprachliche Arbeit** — die Datenseite ist fertig und ändert sich dadurch nicht.
6. ~~**Paket C (Alt-Paket):** Obituary-Text fehlt noch im Nekrolog-Tab~~ — **eingebaut v0.9.17.31** (`_necroObit`, gleiche Regeneration wie Off-Season-Modal). Bank zuvor erweitert v0.9.17.29 (174 → 256 Zeilen).
7. ~~**Paket 7:** N5/N7/N8~~ — **komplett abgeschlossen.** Die drei waren **Prüfaufträge an Opus**, keine Text-Edits (deshalb standen sie nie in den Fix-Commits): **N5 + N7 v0.9.17.34** (pit-`{pos}` = Position beim Reinkommen, festgeschrieben; `{deadRaceBei}` löst den Kasusbruch bei deutschen Rennnamen), **N8 v0.9.17.35** (siegloser Champion fällt aus dem `dominant`-Pool).
